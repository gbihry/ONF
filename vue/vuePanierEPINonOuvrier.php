    ...
    <div class="panier">
        ...
        if ($ligneCommandeEPI == false){
            ...
            foreach ($ligneCommandeEPI as $ligneCommandeUnique) {
                ...
                $maxType = ModeleObjetDAO::getQuantiteEpi($_SESSION['login'],$ligneCommandeUnique['idType'])['sum(quantite)'];
                    echo ("<div class='content'>
                        ...
                        if ($maxQuantite > ($max - $maxType)){
                            echo ("<div id='quantite' data-max='".($max - ($maxType - $quantite))."' data-ligne='". $idLigne ."' data-data='".$quantite . "'><span>" .$maxType . "</span><div class='clear'></div><a class='edit_btn' onclick='edit(this,\"quantiteEPI\")' name='edit_btn'><i class='fa-solid fa-pencil'></i> Modifier</a></div>");
                        }else{
